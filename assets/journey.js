(()=>{
  ...
  const DAYS=[
    ...
    {day:4,title:"Como sua cabeça resolve?",subtitle:"Raciocínio e solução de problemas",icon:"brain",implemented:false},
  ...
  const fresh=()=>({completedDays:[],xp:0,emotionIndex:0,emotionAnswers:[],emotionResult:null,wheelIndex:0,wheelAnswers:[],wheelResult:null});
  function load(){try{const x=JSON.parse(localStorage.getItem(JOURNEY_KEY)||"null");return x&&typeof x==="object"?{...fresh(),...x,completedDays:Array.isArray(x.completedDays)?x.completedDays:[]}:fresh()}catch{return fresh()}}
  const state=load();
  ...
  ensureDay1();renderJourney();if(state.emotionResult)renderEmotionResult();if(state.wheelResult)renderWheelResult();
})();
